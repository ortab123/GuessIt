...
export default function AnalyticsTab() {
  const { childList, seedMockStats } = useChildren();
  ...
  const handleSeed = async () => {
    setSeedMsg("");
    const err = await seedMockStats();
    setSeedMsg(err ? `❌ ${err}` : "✅ Mock data seeded!");
  };
    ...
    <div className="bg-white/60 backdrop-blur-md rounded-2xl p-6 shadow-lg border border-white/30">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={handleSeed}
            className="px-4 py-2 rounded-lg bg-gradient-to-r from-indigo-500 to-blue-500 text-white hover:from-indigo-600 hover:to-blue-600 shadow-md hover:shadow-lg transition"
          >
            Seed mock data
          </button>
