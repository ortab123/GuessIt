    ...
    <div className="space-y-3">
      {cards.map((child) => {
        ...
        return (
          <div
            key={child.id}
            className="bg-white/70 rounded-xl border border-white/30 shadow p-4"
          >
            <button
            ...
            {openChildId === child.id && (
              <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-3">
                <StatCard
                  ...
                  value={(child.timeMin / 60).toFixed(1)}
                />
                ...
                <StatCard
                  title="Difficult Breed"
                  value={child.difficult_breed || "-"}
                />
              </div>
            )}
          </div>
  ...
  return (
    <div className="bg-white/80 rounded-xl border border-white/30 shadow p-4 flex flex-col items-center justify-center">
      ...
      <div className="text-xl text-gray-800 text-center space-y-1 max-w-[150px] break-words">
        {Array.isArray(value)
          ? value.map((breed, idx) => <div key={idx}>{breed}</div>)
          : value}
      </div>
    </div>
  );
